...
public class MartialMoralityEyeRender extends DynamicRender<IMachineFeature, MartialMoralityEyeRender> {
    ...
    public void render(IMachineFeature feature, float v, PoseStack poseStack, MultiBufferSource buffer, int i, int i1) {
        ...
        // Celestial LARGE_COGWHEEL = new Celestial(AllBlocks.LARGE_COGWHEEL.asStack(), 2.4, 1.4, 2.6, false, 1.8, 0.34,
        // List.of(COGWHEEL));
        // Celestial GEARBOX = new Celestial(AllBlocks.GEARBOX.asItem().getDefaultInstance(), 3.7, 1.32, 0.8, false,
        // 1.7, -0.23, null);
        // Celestial MANA_STEEL_GEARBOX_CASING = new
        // Celestial(CTNHBlocks.CASING_MANASTEEL_GEARBOX.asItem().getDefaultInstance(), 0.35, 2.5, 3.0, false, 0.3, 0.1,
        // null);
        // Celestial ZENITH_CASING_GEARBOX = new
        // Celestial(CTNHBlocks.ZENITH_CASING_GEARBOX.asItem().getDefaultInstance(), 6.5, 0.8, 1.0, false, 4.0, -0.13,
        // List.of(MANA_STEEL_GEARBOX_CASING));
        // List<Celestial> planets = List.of(LARGE_COGWHEEL, GEARBOX, ZENITH_CASING_GEARBOX);
        // var metaMachine = feature.self();
        // if (metaMachine instanceof MartialMoralityEyeMachine machine && machine.isFormed() && (machine.isActive() ||
        ...
        // ClientUtil.renderStatic(AllBlocks.BRASS_CASING.asStack(), ItemDisplayContext.GROUND, 15728880,
        // OverlayTexture.NO_OVERLAY, poseStack, buffer, level, 0);
        // poseStack.popPose();
        // //渲染行星及其卫星
        // planets.forEach((planet) -> renderCelestial(planet, null, poseStack, time, tiltRad,
        // metaMachine.getHolder().self(), buffer));
        // poseStack.popPose();
        // }
    }
    ...
    public void renderCelestial(Celestial celestial, @Nullable BlockPos parentWorldPos, PoseStack poseStack, float time,
                                double tiltRad, BlockEntity blockEntity, MultiBufferSource buffer) {
        ...
        double worldY = (parentWorldPos != null ? parentWorldPos.getY() : blockPos.getX() + 0.5) +
                orbitPosLocal.y * cosTilt - orbitPosLocal.z * sinTilt;
        double worldZ = (parentWorldPos != null ? parentWorldPos.getZ() : blockPos.getX() + 0.5) +
                orbitPosLocal.y * sinTilt + orbitPosLocal.z * cosTilt;
        ...
        if (celestial.satellites != null && !celestial.satellites.isEmpty()) {
            celestial.satellites.forEach(
                    satellite -> renderCelestial(satellite, new BlockPos((int) worldX, (int) worldY, (int) worldZ),
                            poseStack, time, tiltRad, blockEntity, buffer));
        }
        poseStack.popPose();
    }

    private Quaternionf calculateSelfRotationQuaternion(Celestial celestial, float time) {
        float angleSelf = (float) (celestial.tidallyLocked ? -(time * celestial.orbitSpeed) % 360 :
    ...
    public class Celestial {
        ...
        public Celestial(ItemStack itemStack, double orbitRadius, double orbitSpeed, double selfRotationSpeed,
                         boolean tidallyLocked, double scale, double height, @Nullable List<Celestial> satellites) {
