...
public class ClientUtil {
    ...
    public static void renderStatic(ItemStack itemStack, ItemDisplayContext itemDisplayContext, int combinedLight,
                                    int combinedOverlay, PoseStack poseStack, MultiBufferSource bufferSource,
                                    @Nullable Level pLevel, int pSeed) {
        Minecraft.getInstance().getItemRenderer().renderStatic(itemStack, itemDisplayContext, combinedLight,
                combinedOverlay, poseStack, bufferSource, pLevel, pSeed);
    }
    ...
    public static void renderModel(PoseStack poseStack, MultiBufferSource buffer, ResourceLocation resourceLocation) {
        ...
        Minecraft.getInstance().getBlockRenderer().getModelRenderer().renderModel(poseStack.last(),
                buffer.getBuffer(RenderType.solid()), null, model, 1.0F, 1.0F, 1.0F, LightTexture.FULL_BRIGHT,
