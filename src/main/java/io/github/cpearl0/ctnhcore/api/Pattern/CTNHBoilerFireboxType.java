...
public record CTNHBoilerFireboxType(String name, ResourceLocation bottom, ResourceLocation top, ResourceLocation side) {
    ...
    static {
        CODEC = Codec.STRING.comapFlatMap((name) -> {
            ...
            return type != null ? DataResult.success(type) :
                    DataResult.error(() -> "A firebox type named " + name + " does not exist");
        }, BoilerFireboxType::name);
        NAQUADAH_FIREBOX = new BoilerFireboxType("naquadah_firebox", CTNHCore.id("block/casings/nq_casing"),
                CTNHCore.id("block/casings/nq_casing"),
                CTNHCore.id("block/casings/firebox/machine_casing_firebox_naquadah"));
    }
}
