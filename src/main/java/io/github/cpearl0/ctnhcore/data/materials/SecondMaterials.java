...
public class SecondMaterials {
    ...
    public static void init() {
        ...
        RP1RocketFuel = Builder("rp_1_rocket_fuel").fluid().color(0x9E2A2A).iconSet(MaterialIconSet.DULL)
                        .buildAndRegister();

        Kerosene = Builder("kerosene").fluid().color(0x752275).iconSet(MaterialIconSet.DULL).buildAndRegister();

        DenseHydrazineMixedFuel = Builder("dense_hydrazine_mixed_fuel").fluid().color(0x833D59).iconSet(MaterialIconSet.DULL)
                        .buildAndRegister();
        ...
        EthylAnthraHydroQuinone = Builder("ethyl_anthra_hydro_quinone").fluid().color(0xC9E08D).iconSet(MaterialIconSet.DULL)
                        .buildAndRegister();

        Anthracene = Builder("anthracene").fluid().color(0xBBBABA).iconSet(MaterialIconSet.DULL).buildAndRegister();

        MethylhydrazineNitrateRocketFuel = Builder("methylhydrazine_nitrate_rocket_fuel").fluid().color(0x613B87)
                        .iconSet(MaterialIconSet.DULL).buildAndRegister();

        MethylHydrazine = Builder("methyl_hydrazine").fluid().color(0x606060).iconSet(MaterialIconSet.DULL)
                        .buildAndRegister();

        UDMHRocketFuel = Builder("udmh_rocket_fuel").fluid().color(0x2AA327).iconSet(MaterialIconSet.DULL)
                        .buildAndRegister();

        UDMH = Builder("udmh").fluid().color(0x050543).iconSet(MaterialIconSet.DULL).buildAndRegister();

        OrangeMetal = Builder("orange_metal").dust().color(0xfa7e23).iconSet(MaterialIconSet.ROUGH)
                        .buildAndRegister();

        PhthalicAnhydride = Builder("phthalic_anhydride").dust().color(0x6C863A).iconSet(MaterialIconSet.ROUGH)
                        .buildAndRegister();
        ...
        PotassiumPyrosulfate =
                Builder("potassium_pyrosulfate").dust().fluid(FluidStorageKeys.MOLTEN, new FluidBuilder())
