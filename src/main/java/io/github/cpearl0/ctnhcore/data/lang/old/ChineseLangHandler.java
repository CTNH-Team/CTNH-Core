...
public class ChineseLangHandler {
    ...
    public static void init(RegistrateCNLangProvider provider) {
        ...
        provider.add("item.ctnh.me_advanced_terminal.setting.4.tooltip",
                "是否启用线圈替换模式(0:不启用,1:启用)\n启用线圈替换模式会将所有线圈替换为指定等级的线圈\n请确保物品栏中有空间存放替换下来的线圈");
        provider.add("item.ctnh.me_advanced_terminal.setting.5", "使用AE存储");
        provider.add("item.ctnh.me_advanced_terminal.setting.5.tooltip",
                "是否启用AE库存(0:不启用,1:启用)\n启用后将优先在AE库存中检索\n通过ME无线访问点链接到网络");
        provider.add("item.ctnh.me_advanced_terminal.setting.6", "放置流体");
        provider.add("item.ctnh.me_advanced_terminal.setting.6.tooltip",
                "是否启用流体放置(0:不启用,1:启用)\n启用后将检索并消耗物品栏/背包流体容器中的或AE库存中的流体\n可堆叠的流体容器须保证堆叠数为1");
        provider.add("item.ctnh.me_advanced_terminal.setting.7", "在流体中放置");
        provider.add("item.ctnh.me_advanced_terminal.setting.7.tooltip",
                "是否在流体中放置方块(0:不启用,1:启用)\n启用后会将空间中的流体视为空位\n与“放置流体”同时启用时，不会在流体中放置流体");
        provider.add("item.ctnh.me_advanced_terminal.setting.8", "拆除模式");
        provider.add("item.ctnh.me_advanced_terminal.setting.8.tooltip",
        ...
        provider.add("ctnh.item.dyson_tier1", "集成性光伏无人机蜂群");
        provider.add("ctnh.item.dyson_tier2", "§4我们的蜂群遮天蔽日");
        provider.add("ctnh.item.nuclear_reactor_heat", "基础堆温：%d°C");
        provider.add("ctnh.item.terminal.success_get", "已经获取坐标!");
        provider.add("ctnh.item.terminal.success_write", "已成功写入坐标!");
        provider.add("ctnh.item.terminal.location", "已经绑定的坐标：(%s,%s,%s)");
        provider.add("ctnh.item.terminal.success_clear", "已清除坐标！");
        provider.add("ctnh.item.terminal.tips", "使用右键绑定光伏模块控制器，然后再右键将控制器和光伏基站绑定\nshift+右键任意方块清除坐标");
        ...
        // research_data
        provider.add("ctnh.item.data.tip1", "当前公式: a%s+b%s+c%s+d");
        ...
        provider.add("ctnh.neuro_matrix_compiler.info.state.working", "§9运行中:%ss/%ss");
        provider.add("ctnh.neuro_matrix_compiler.info.state.finish", "§a完成");
        provider.add("ctnh.neuro_matrix_compiler.info.state.waiting", "§b等待中......");
        ...
        provider.add("ctnh.multiblock.demon_will_generator.tooltip.1",
                "利用机器两侧的区块内的恶魔意志浓度差发电，浓度差与发电量呈指数关系，当浓度差超过500时，超过500的浓度与发电量改为线性关系");
        ...
        provider.add("ctnh.multiblock.fermenting_tank.tooltip.2",
                "当发酵罐温度处于§236§r至§238§r度之间时为最适生长温度，配方获得1.2倍效率，越偏离最适生长温度，配方效率越低，最低为三分之一");
        provider.add("ctnh.multiblock.fermenting_tank.tooltip.3",
                "微生物的生长符合逻辑斯蒂方程，当输入仓内液体体积为容量的一半时，§2生长效率达到两倍§r，而满仓和空仓时生长效率最低，保底为20%");
        ...
        provider.add("ctnh.multiblock.large_fermenting_tank.tooltip.1",
                "可接入附属结构，在对应位置连接上一个大发酵瓶后，可以根据发酵瓶中的液体种类提升保底效率：水(50%)，简易培养基(150%)，无菌培养基(200%)");
        ...
        provider.add("ctnh.multiblock.digestion_tank.tooltip.2",
                "当化粪池温度处于§236§r至§238§r度之间时为最适生长温度，配方获得1.2倍效率，越偏离最适生长温度，配方效率越低，最低为三分之一");
        ...
        provider.add("ctnh.multiblock.underfloor_heating_system.tooltip.1",
                "地暖系统依靠蒸汽供暖，占地一个区块，能对§a周围5*5的区块§r产生供暖，供暖只会在地暖上方十格内生效");
        ...
        provider.add("ctnh.multiblock.industrial_primitive_blast_furnace.tooltip.1",
                "工业土高炉在持续运行配方时，会不断升温，而一旦中止，则会迅速冷却");
        ...
        provider.add("ctnh.multiblock.quasar_eye.tooltip.10",
                "在创生模式下释放所有积累的电量，使用高级燃料可以使输出获得倍乘。同时每积累1000E EU就额外产出一份气体产出,积累电量小于1E时无法启动创生模式");
        ...
        provider.add("ctnh.multiblock.photovoltaic_power_station_energetic.tooltip.2",
                "只在白天工作，不同维度会对太阳能发电的效率产生影响，基础产能功率为在主世界正午的功率");
        ...
        provider.add("ctnh.multiblock.wide_accelerator.tooltip.3",
                "本机器只要求粒子速度大于配方需求，不满足需求无法运行。粒子速度不会超过50Gev，这台机器非常安全，不会引发爆炸。");
        ...
        provider.add("ctnh.multiblock.wide_accelerator.tooltip.7",
                "本机器可以存储至多50E EU能量，存储的能量可以在加速界面调整三种粒子速度，每调整1M ev粒子速度需要消耗10M EU存储的能量，按住shift时可以一次性更改10M ev粒子速度，按住ctrl时可以一次性更改100M ev粒子速度");
        provider.add("ctnh.multiblock.wide_accelerator.tooltip.8",
                "三种粒子轨道速度之和每有1M ev，每tick便消耗机器存储的100EU能量，电量不足时，每tick粒子速度会衰减10%");
        provider.add("ctnh.multiblock.wide_accelerator.tooltip.9",
                "机器没有运行时，默认使用存储能源舱室的所有能源为机器充能。机器运行时仍然使用能源仓的能量。但在机器运行时除非机器存储电量不够，否则能源仓不会为机器充能");
        provider.add("ctnh.multiblock.wide_accelerator.tooltip.10",
                "注意:本机器用电量极高，且暂时无法做到只能计算正确并行，使用低电压可能导致§c配方无法运行§r或者§c跳电§r,建议搭配§9激光仓§r使用，如遇配方不工作，请报告给作者");
        provider.add("ctnh.multiblock.wide_accelerator.tooltip.11",
                "可以与约束器链接传递部分粒子。§c警告：如果没有链接约束器，不要随意尝试某些危险的配方§r (目前还是饼)");
        ...
        provider.add("ctnh.multiblock.arcgenerator.tooltip.2",
                "该机器必须配合电弧生成器使用，要求电弧生成器必须在该机器主方块上方5格，当完成链接时，电弧生成器会显示已完成连接");
        provider.add("ctnh.multiblock.arcgenerator.tooltip.3", "当电弧强度小于配方最小电弧强度时，配方将无法运行");
        provider.add("ctnh.multiblock.arcgenerator.tooltip.4",
                "当电弧强度大于配方最大电弧强度时，配方将以(机器电弧强度-配方需求电弧强度)/(满功率需求电弧强度-需求电弧强度)的效率运行。效率允许超过100%，但不能超过机器最大发电效率");
        ...
        provider.add("ctnh.multiblock.arcreactor.tooltip.1",
                "该机器必须配合电弧撕裂者使用，要求电弧生成器必须在电弧撕裂者主方块上方5格，当完成链接时，电弧生成器会显示桥接已启用");
        ...
        provider.add("ctnh.multiblock.plasma_alloy.tooltip.2",
                "线圈温度每有1800K，获得4点并行，线圈温度超过10000K时，获得(线圈温度-10000)/10000的额外加速");
        ...
        provider.add("ctnh.multiblock.plasma_alloy.tooltip.8",
                "压缩精金等离子：消耗固定100等离子体，使速度*5,使消耗电压翻倍（§c这可能导致配方不运行，请使用多安能源仓）");
        ...
        provider.add("ctnh.multiblock.hybrid_mixer.tooltip.2",
                "机器真实电压等级为配方电压等级和应力等级的较小值。应力输入仓要求转速至少为64，应力输入仓转速为256时，应力等级+1");
        ...
        provider.add("ctnh.multiblock.spacephotovoltaicbasestation.tooltip.2",
                "在太空发电模式下，星球类型和光伏方块的等级都会提升发电量，在空间站被视为无重力环境，且发电量*4,消耗特定材料以进一步提升发电量");
        provider.add("ctnh.multiblock.spacephotovoltaicbasestation.tooltip.3",
                "在太空光伏组装模式下，不消耗EUt，发电量将锁定为1，根据配方的模拟F功率来计算速度和并行量");
        provider.add("ctnh.multiblock.spacephotovoltaicbasestation.tooltip.4",
                "光伏等级，光照强度共同决定了是否可以执行太空组装配方，光伏方块耐热性和耐热结构方块决定了可以获得的光照最大倍率，太空结构方块决定了可以使用的光伏方块等级和是否可以使用附属结构");
        provider.add("ctnh.multiblock.spacephotovoltaicbasestation.tooltip.5",
                "在太空光伏组装模式下，最终并行为(太空发电模式下发电量/模拟功率),最终时间倍率为(模拟功率/太空发电模式下发电量)，当太空发电模式下发电量小于模拟功率时，最终时间倍率将变为平方");
        ...
        provider.add("ctnh.multiblock.lasersorter.tooltip.6",
                "输入的算力每比基础算力多一倍，将一次超频转化为无损超频（即运行速度*2），该效果转化的次数不超过你能超频的等级（即上限为将你所有的有损超频转化为无损）");
        ...
        provider.add("ctnh.multiblock.lasersorter.tooltip.10",
                "输入的算力每比基础算力多一倍，将一次超频转化为无损超频（即运行速度*2），该效果转化的次数不超过你能超频的等级（即上限为将你所有的有损超频转化为无损）");
        ...
        provider.add("ctnh.multiblock.pvdrone.tooltip.3",
                "将无人机放入无人机支架以开始发送无人机，每5秒和运行结束时，每个无人机都有一定概率消耗，无人机发电同样受维度和空间站增幅");
        ...
        provider.add("ctnh.nuclear_reactor.coolant",
                "冷却液可以使用蒸汽（150°C），氘（450°C），钠（800°C），钠钾合金（900°C），反应的堆温越高，消耗冷却液的速度越快，冷却液的热容越大，消耗速度越慢");
        ...
        provider.add("ctnh.multiblock.cryotheum_freezer.tip.2",
                "初始具有4并行和3泪之晶点数，可以在升级界面加点。每消耗10000mb凛冰，就获得一点点数，随后将目标翻四倍");

        provider.add("ctnh.multiblock.neuro_martix_compiler.tip.0",
                "将生物的进化之道完全放任于碳基生物的自然演变是一种低效且缓慢的做法，现在我们将亲自编码每一个基因序列，将我们的至臻完美编译在神经元的逻辑之中");
        provider.add("ctnh.multiblock.neuro_martix_compiler.tip.01",
                "神经矩阵编码器（CMP）是一台编译神经序列的机器，其不同于其他机器，不执行正常的输入逻辑，无法超频");
        ...
        provider.add("ctnh.multiblock.neuro_martix_compiler.tip.5",
                "每次检测到新的配方时，机器将§6完美诉诸于随机§r，生成函数F(x1,x2,x3)=§6y=ax1+bx2+cx3+d§r，其中，x1,x2,x3为期望的片区所消耗的物品数量，同时在给定范围内随机x1,x2,x3,获取答案y");
        ...
        provider.add("ctnh.multiblock.neuro_martix_compiler.tip.part3",
                "片区5：此片区用为神经编译提供电路板支持，决定噪声ϵ波动，如果提供电路板大于配方给定值则不造成噪声影响");
        provider.add("ctnh.multiblock.neuro_martix_compiler.tip.part4", "片区6：收集最终编译结果的片区，在完成一次逻辑运算流程后，将根据结果对神经数据集进行修改");
        provider.add("ctnh.multiblock.neuro_martix_compiler.tip.7",
                "在片区1-5执行完毕后，进行持续5s的总计算流程，在此过程中给出x1,x2,x3，计算得到计算值y，与真实比较，进行最终编译运算");
        provider.add("ctnh.multiblock.neuro_martix_compiler.tip.8", "————————最终编译运算————————");
        provider.add("ctnh.multiblock.neuro_martix_compiler.tip.9",
                "最终编译运算将比较真实y与结果y，如果结果y的值在真实值y的0.9-1.1倍内，则运算成功，将编译数据集变为配方输出");
        provider.add("ctnh.multiblock.neuro_martix_compiler.tip.10",
                "如果运算失败，则定义噪声结果函数f(x1,x2,x3)=ax1+bx2+cx3+d+ϵ,根据噪声决定片区，噪声波动最多翻倍5倍，片区误差项为0.9-1.1间，则噪声变为0.5倍率");
        provider.add("ctnh.multiblock.neuro_martix_compiler.tip.11",
                "最终编译数据集将获得三个信息，信息1代表本次的方程，信息2代表噪声结果函数获得的结果值对于y的倍率，信息3代表误差项的比率");
        ...
        provider.add("ctnh.u_sinope.7",
                "对于常规配方，该巨构拥有8^（电压等级）的并行，最高不超过2^32，在能源仓等级达到OPV时解锁无损超频，配方等级每超过UHV一级，处理速度+555%,每100点并行使处理速度增加333%,如果使用了四维工程学材料，则速度额外增加5000%");
        ...
        for (int tier : GTValues.tiersBetween(LV, HV)) {
            provider.add(CTNHMachines.ENERGY_OUTPUT_HATCH_4A_LOWER[tier].getBlock(),
                    "4安" + GTValues.VNF[tier] + "§r动力仓");
        }
        ...
        provider.add(CTNHMachines.DEHYDRATOR[HV].getBlock(), "§6高级脱水机 II§r");
        provider.add(CTNHMachines.DEHYDRATOR[EV].getBlock(), "§5高级脱水机 III§r");
        provider.add(CTNHMachines.DEHYDRATOR[IV].getBlock(), "§9精英脱水机 §r");
        provider.add(CTNHMachines.DEHYDRATOR[LuV].getBlock(), "§d精英脱水机 II§r");
        provider.add(CTNHMachines.DEHYDRATOR[ZPM].getBlock(), "§c精英脱水机 III§r");
        provider.add(CTNHMachines.NAQUADAH_REACTOR[EV].getBlock(), "§5高级硅岩发电机 I");
        provider.add(CTNHMachines.NAQUADAH_REACTOR[IV].getBlock(), "§9精英硅岩发电机 II");
        provider.add(CTNHMachines.NAQUADAH_REACTOR[LuV].getBlock(), "§d精英硅岩发电机 III");
        provider.add(CTNHMachines.NAQUADAH_REACTOR[ZPM].getBlock(), "§c精英硅岩发电机 IV");
        provider.add(CTNHMachines.NAQUADAH_REACTOR[UV].getBlock(), "§3终极硅岩发电机 V");
