    ...
    public boolean onWorking() {
        if (getOffsetTimer() % 20 == 0) {
            ...
            if (machine1 instanceof MultiblockTankMachine tankMachine) {
                ...
                if (tank.getFluidInTank(0).getFluid().equals(Fluids.WATER) &&
                        tank.getFluidInTank(0).getAmount() >= 100) {
                    tank.getFluidInTank(0).setAmount(tank.getFluidInTank(0).getAmount() - 100);
                    Lower_limit = 0.5;
                } else if (tank.getFluidInTank(0).getFluid().equals(GTMaterials.SterileGrowthMedium.getFluid()) &&
                        tank.getFluidInTank(0).getAmount() >= 100) {
                            tank.getFluidInTank(0).setAmount(tank.getFluidInTank(0).getAmount() - 100);
                            Lower_limit = 2;
                        } else
                    if (tank.getFluidInTank(0).getFluid().equals(CTNHMaterials.SimpleGrowthMedium.getFluid()) &&
                            tank.getFluidInTank(0).getAmount() >= 100) {
                                tank.getFluidInTank(0).setAmount(tank.getFluidInTank(0).getAmount() - 100);
                                Lower_limit = 1.5;
                            } else {
