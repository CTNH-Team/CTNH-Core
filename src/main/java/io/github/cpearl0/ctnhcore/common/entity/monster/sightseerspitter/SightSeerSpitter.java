...
public class SightSeerSpitter extends Mob implements Enemy {
    ...
    static class SightSeerSpitterSurroundTargetGoal extends Goal {
        ...
        public boolean canContinueToUse() {
            ...
            return canUse() && Math.abs(pos.x - targetPos.x) > 0.1 && Math.abs(pos.y - targetPos.y) > 0.1 &&
                    Math.abs(pos.z - targetPos.z) > 0.1 && mob.position().distanceToSqr(targetPos) > 0.09 // 0.3 * 0.3
        ...
        public void tick() {
            ...
            List<Player> entities = mob.level().getEntities(EntityType.PLAYER,
                    mob.getBoundingBox().expandTowards(targetMotion).inflate(0.15), e -> !e.isSpectator());
