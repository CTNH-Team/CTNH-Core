...
public class CreativeEnergyHatchPartMachine extends TieredIOPartMachine implements IDataInfoProvider {
    ...
    public List<Component> getDataInfo(PortableScannerBehavior.DisplayMode mode) {
        if (mode == PortableScannerBehavior.DisplayMode.SHOW_ALL ||
            ...
            return Collections.singletonList(Component.literal(
                    String.format("%d/%d EU", energyContainer.getEnergyStored(),
    ...
    private static class InfinityEnergyContainer extends NotifiableEnergyContainer {
        ...
        public InfinityEnergyContainer(MetaMachine machine, long maxCapacity, long maxInputVoltage,
        ...
        public boolean handleRecipe(IO io, GTRecipe recipe, List<Long> left, boolean simulate) {
            return super.handleRecipe(io, recipe, left, true);
        }

        @Override
        public long changeEnergy(long energyToAdd) {
            long oldEnergyStored = getEnergyStored();
            long newEnergyStored = (getEnergyCapacity() - oldEnergyStored < energyToAdd) ? getEnergyCapacity() :
