...
public class IDataItem extends ComponentItem implements IInteractionItem {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltipComponents,
        ...
        if (nbt.contains("formula")) {
            ...
            tooltipComponents.add(Component.translatable("ctnh.item.data.tip1", String.format("%d", formula[0]),
                    String.format("%d", formula[1]), String.format("%d", formula[2])));
        }
        if (nbt.contains("muti")) {
            tooltipComponents
                    .add(Component.translatable("ctnh.item.data.tip2", String.format("%.2f", nbt.getDouble("muti"))));
        }
        if (nbt.contains("noise")) {
            tooltipComponents
                    .add(Component.translatable("ctnh.data.noise", String.format("%.2f", nbt.getDouble("noise"))));
        }
