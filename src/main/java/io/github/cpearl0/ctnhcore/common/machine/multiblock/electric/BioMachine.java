...
public class BioMachine extends WorkableElectricMultiblockMachine {
    ...
    public void addDisplayText(List<Component> textList) {
        ...
        if (Machine_Temperature >= 36 && Machine_Temperature <= 38) {
            ...
            Efficiency = 1 /
                    Math.min(3, Math.pow(Math.max(36 - Machine_Temperature, Machine_Temperature - 38), 2) / 10 + 1);
        }
        super.addDisplayText(textList);
        textList.add(textList.size(),
                Component
                        .translatable("ctnh.fermenting_tank.growing_temperature",
                                String.format("%.1f", Machine_Temperature))
                        .setStyle(Style.EMPTY.withColor(ChatFormatting.GREEN)));
        textList.add(textList.size(), Component.translatable("ctnh.fermenting_tank.growth_efficiency",
    ...
    public static ModifierFunction recipeModifier(MetaMachine machine, GTRecipe recipe) {
        ...
        dmachine.Machine_Temperature = getWorldTemperature(Objects.requireNonNull(dmachine.getLevel()),
                dmachine.getPos());
        if (dmachine.Machine_Temperature >= 36 && dmachine.Machine_Temperature <= 38) {
            ...
            dmachine.Efficiency = 1 / Math.min(3,
                    Math.pow(Math.max(36 - dmachine.Machine_Temperature, dmachine.Machine_Temperature - 38), 2) / 10 +
                            1);
        }
